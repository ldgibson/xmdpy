...
class OnDiskXYZTrajectory:
    ...
    def __post_init__(self) -> None:
        self.n_frames, self.atoms = self.get_xyz_dims_and_details()
        self.n_atoms = len(self.atoms)

    def get_xyz_dims_and_details(self) -> tuple[int, list[str]]:
        n_lines = count_lines(self.filename)
    ...
    def get_positions(
        ...
        offset = 2
        lines_per_frame = self.n_atoms + offset
        ...
        positions = np.zeros((len(frames), len(atoms), 3), dtype=self.dtype)
    ...
    def positions(self) -> OnDiskArray:
        return OnDiskArray(self.get_positions, (self.n_frames, self.n_atoms, 3))
